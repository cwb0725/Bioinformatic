...
class myUnet(object):
    ...
    def train(self):
        ...
        imgs_train = np.load("./npy_data/imgs_train.npy").astype("float32")
        imgs_mask_train = np.load("./npy_data/imgs_mask_train.npy").astype("float32")
        ...
        os.makedirs("../npydata", exist_ok=True)
        np.save("../npydata/train_mean.npy", mean.astype(np.float32))
        imgs_train -= mean
        ...
        # ---- Patch 训练参数----
        PATCH = 512
        STEPS_PER_EPOCH = 200       # 每个 epoch 抽 200 个 patch
        VAL_STEPS = 50              # 验证抽 50 个 patch
        EPOCHS = 5

        # 前景偏置：80% 采样尽量含线粒体
        P_FG = 0.8
        MIN_FG_RATIO = 0.002        # patch 内前景>=0.2%就算含前景
        MAX_TRIES = 30
